...
def start_meeting_as_host():
    try:
        ...
        # Wait for the Zoom windows to appear
        root.after(10000, handle_zoom_windows)  # Wait 10 seconds to ensure Zoom windows are open
...
def handle_zoom_windows():
    try:
        # Close the 'Zoom Workplace - Free account' window
        close_zoom_cmd = "wmctrl -c 'Zoom Workplace - Free account'"
        subprocess.Popen(close_zoom_cmd, shell=True)

        # Wait for the Zoom meeting window to appear
        time.sleep(2)

        # Adjust the Zoom meeting window
        ...
        # Set the new width to 78% of screen width and position at the left edge of the screen
...
# Update left frame to light sky blue and add a dark gray border with 20px thickness
...
# Start meeting button with updated styling
...
# Prescription buttons with updated styling
for i in range(1, 4):
...
# Create a frame for the exit button and image, place it at the bottom
...
image_label.pack(pady=(0, 5))  # Adjust padding if necessary

# Exit button to close the application, positioned at the bottom right corner
